...
class Reactor {
  ...
  constructor(port: typeof parentPort) {
    ...
    this.dynamicHandlers.set('call', async (
        transfer: any[],
  ...
  async handleMessage(ev: any) {
    switch (ev.type) {
      case 'init': return await this.handleInit(ev);
      case 'invoke': return await this.handleInvoke(ev);
    }
  }
  ...
  async handleInit(ev: InternalConfig & {
    type: string,
    names: string[],
    modules: ArrayBuffer[],
    ...
    const functions = Object.fromEntries(Object.entries(ev.functions).map(([namespace, funcs]) => {
      return [namespace, Object.fromEntries(funcs.map(funcName => {
        return [funcName, (context: CallContext, ...args: any[]) => this.callHost(context, namespace, funcName, args)]
      }))]
    }))
